...
### 변수에 저장할수 있는 값의 종류 → 자바스크립트는 총 7개 (기본 타입)
...
### 입력받은 매개변수 값을 3배에서 return 해주는 함수가 있다.
...
객체의 실제 타입보다는 그 행동(메서드,속성)에 따라 사용가능 여부를 판단하는 방식이다.

오리처럼 걷거나 꽥꽥거리면 그건 오리다.

### Duck Typing

아래와 같은 자바스크립트 함수가 있다고 가정했을때, thing이라는 매개변수로써 오는 값의 타입은 중요하지 않다. 그저 그 값에 행동(메서드)가 존재하기만 하면 오케이다.
...
아래와 같이 극단적으로 match 하지 않아도 오류는 나지 않는다.
...
# 그렇다면 타입스크립트의 구조적 타이핑 방식은 무조건 안전할까?
...
자바스크립트 공부를 처음하면 공부하는 내용인, 값 (value)는 쉽게 말하면 메모리에 저장한 모든 데이터이다.
...
먼저, 타입스크립트에서 값은 당연히 존재한다. 또한 타입이라는 새로운 개념도 등장했다.

타입스크립트에서 둘의 공간은 분리되어있다. 즉, 같은 이름의 값과 타입은 공존이 가능하다.

자바스크립트의 슈퍼셋이라고 유식하게 표현할 수 있겠다.

둘을 어떻게 구별하냐고? type은 : 로 타입 지정을 하고 값은 =로 할당을 한다.
...
**정답: 완전히 일치하지 않아도 됩니다. “속성만 포함”하면 OK다.**

### 위에서 언급했던 구조적 타이핑 생각하면 딱이다.

속성 구조가 같거나, 오히려 더 많아서 과분하거나.

    **타입스크립트는 구조적 타입 시스템이라 “일치가 아닌 포함”을 본다**
...
**typeof [] → object다. Array 아니야!**

typeof function(){} → function (그러나 내부적으로는 Array)
...
실제 상황에서 둘다 자주 쓰이기에 뭐가 더 좋다 라고 말할순 없지만, 개인적으로는 재사용성은 호출시그니처가 좀 더 좋은거같다.
